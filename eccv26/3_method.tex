...
\subsection{Overview}
The proposed framework shown in ~\figref{main_frame} operates in three stages: motion prediction, token sparsification, and target decoding. Given an input video frame and the target's initial location, a pair of images is processed as input: a template image denoted as $Z \in \mathbb{R}^{3 \times H_{z} \times W_{z}}$ and a search image denoted as $X \in \mathbb{R}^{3 \times H_{x} \times W_{x}}$. These images are divided into non-overlapping patches of size $P \times P$, resulting in $P_{z} = \frac{H_{z} \times W_{z}}{P^2}$ patches for the template and $P_{x} = \frac{H_{x} \times W_{x}}{P^2}$ patches for the search region. After patch embedding concatenated together, features are simultaneously extracted and fusion in the form of sequence using a Transformer encoder, in which the sequence will be first handled by our proposed sparsification block. In the end, the retained tokens will be further processed by the prediction head.

% In the motion prediction stage, we estimate a motion window that captures the likely location of the target in the current frame, leveraging the predicted target location from the previous frame as a prior. This motion window acts as a spatial constraint to guide the token sparsification process. In the token sparsification stage, redundant tokens outside the motion window are filtered out, while the remaining tokens are re-ranked based on their importance scores. Finally, the target decoding stage employs a fully sparse head, based on 1×1 convolutions, to process the sparsified token representations and produce the final tracking prediction. This design ensures computational efficiency without compromising tracking accuracy.
...
\subsection{Fully Sparse Head and Loss}
...
We design a natively sparse head that directly consumes $\mathcal{L}_K$ without padding or reshaping back to a dense feature map.
We parameterize both branches with lightweight MLPs, similar to prior MLP-based heads (e.g., LoRATrack~\cite{lin2024loratrack}). We further tailor the head for sparse token decoding via a score-first, regress-once computation path.
